...
export class AddressBookComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  async updateTrackedBalances(refresh= false) {
    ...
    for (const entry of this.addressBookService.addressBook) {
      ...
      // Add balances from RPC data
        ...
        balanceAccount.balanceFiat = this.util.ana.rawToAna(balanceAccount.balance).times(this.fiatPrice).toNumber();
  ...
  async exportAddressBook() {
    ...
    const exportUrl = `https://ananault.lightcord.org/import-address-book#${base64Data}`;
  ...
  exportAddressBookToFile() {
    const fileName = `AnaNault-AddressBook.json`;
    ...
    reader.onload = (event) => {
      ...
      try {
        ...
        if (!importData.length || !importData[0].account) {
          return this.notificationService.sendError(`Bad import data, make sure you selected a AnaNault Address Book export`);
        }
