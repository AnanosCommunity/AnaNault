...
export class QrModalComponent implements OnInit {
  ...
  nano_scheme = /^(ana|anarep|anaseed|anakey):.+$/g;
  ...
  onCodeResult(resultString: string) {
    ...
    if (this.util.account.isValidAccount(resultString)) {
      ...
      if (this.util.ana.isValidHash(resultString)) {
        type = 'hash';
        content = resultString;
      }
    } else if (this.util.ana.isValidHash(resultString)) {
      type = 'hash';
      content = resultString;
      ...
      if (['ana:', 'anarep:'].includes(url.protocol) && this.util.account.isValidAccount(url.pathname)) {
        type = 'account';
      } else if (['anaseed:', 'anakey:'].includes(url.protocol) && this.util.ana.isValidHash(url.pathname)) {
        type = 'hash';
      }
    } else {
      type = 'generic';
