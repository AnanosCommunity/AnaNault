  ...
  async processSend(privKey, previous, sendCallback) {
    ...
    if (this.validDestination && nanocurrency.checkAddress(this.destinationAccount.replace(/^ana_/, "nano_"))) {
      ...
      if (data.hash) {
        ...
        (nanoAmountSent ? (nanoAmountSent.toString(10) + ' Ananos') : '') + ' transferred to ' + this.destinationAccount, {length: 15000});
        this.appendLog('Funds transferred ' + (nanoAmountSent ? ('(' + nanoAmountSent.toString(10) + ' Ananos)') : '') + ': ' + data.hash);
  ...
  async processIndexRecursive(privKeys, keyCount) {
    ...
    this.processAccount(privKey, function() {
      ...
      if (keyCount < privKeys.length) {
        ...
        this.appendLog(this.totalSwept + ' Ananos transferred');
        this.notificationService.sendInfo('Finished processing all accounts. ' + this.totalSwept + ' Ananos transferred', {length: 0});
        this.sweeping = false;
      }
    }.bind(this));
  }
