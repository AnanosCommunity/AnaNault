...
export class AccountDetailsComponent implements OnInit, OnDestroy {
  ...
  async loadAccountDetails(refresh= false) {
    ...
    if ((!this.account.error && this.account.pending > 0) || this.account.error) {
      ...
      if (this.settings.settings.minimumReceive) {
        const minAmount = this.util.ana.anaToRaw(this.settings.settings.minimumReceive);
  ...
  syncFiatPrice() {
    ...
    const fiatAmount = this.util.ana.rawToAna(rawAmount)
  ...
  async generateSend() {
    ...
    if (!this.validateAmount()) return this.notifications.sendWarning(`Invalid ANA Amount`);
