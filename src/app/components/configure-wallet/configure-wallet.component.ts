...
export class ConfigureWalletComponent implements OnInit {
  ...
  importOptions = [
    { name: 'Ananos Seed', value: 'seed' },
    { name: 'Ananos Mnemonic Phrase', value: 'mnemonic' },
    { name: 'BIP39 Mnemonic Phrase', value: 'bip39-mnemonic' },
    { name: 'AnaNault Wallet File', value: 'file' },
  ...
  async importLedgerWallet(refreshOnly = false) {
    ...
    if (this.ledger.status === LedgerStatus.NOT_CONNECTED) {
      ...
      return this.notifications.sendWarning(`Failed to connect the Ledger device. Make sure the Ananos app is running on the Ledger. If the error persists: Check the <a href="https://docs.nault.cc/2020/08/04/ledger-guide.html#troubleshooting" target="_blank" rel="noopener noreferrer">troubleshooting guide</a>`, { identifier: 'ledger-error', length: 0 });
    }

    if (this.ledger.status === LedgerStatus.LOCKED) {
      return this.notifications.sendWarning(`Unlock your Ledger device and open the Ananos app to continue`);
    }
  ...
  async confirmWalletOverwrite() {
    ...
    try {
      ...
      if (!this.walletService.isLedgerWallet()) {
        this.notifications.sendInfo(`You can use the 'Manage Wallet' page to backup your Ananos seed and/or mnemonic`);
      }
      return false;
    }
  }
    ...
    if (!this.isNewWallet) {
      if (this.selectedImportOption === 'mnemonic' || this.selectedImportOption === 'seed') {
        if (this.selectedImportOption === 'seed') {
          ...
          if (existingSeed.length !== 64 || !this.util.ana.isValidSeed(existingSeed)) return this.notifications.sendError(`Seed is invalid, double check it!`);
          ...
          try {
            ...
            if (!newSeed || newSeed.length !== 64 || !this.util.ana.isValidSeed(newSeed)) return this.notifications.sendError(`Mnemonic is invalid, double check it!`);
        ...
        if (this.isExpanded && this.keyString.length === 128) {
          ...
          if (!this.util.ana.isValidSeed(this.keyString)) {
            return this.notifications.sendError(`Private key is invalid, double check it!`);
          }
        } else if (this.keyString.length !== 64 || !this.util.ana.isValidSeed(this.keyString)) {
          return this.notifications.sendError(`Private key is invalid, double check it!`);
        }
