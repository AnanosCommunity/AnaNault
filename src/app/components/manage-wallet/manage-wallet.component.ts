...
export class ManageWalletComponent implements OnInit {
  ...
  exportToFile() {
    ...
    const fileName = `AnaNault-Wallet.json`;
  ...
  async exportToCsv() {
    ...
    if (this.invalidCsvCount) {
      if (this.beyondCsvLimit) {
        return this.notifications.sendWarning(`To export transactions above the limit, please use a custom AnaNault server`);
      } else {
    ...
    if (history && history.history && history.history.length > 0) {
      history.history.forEach(a => {
        csvData.push({'account': a.account, 'type': a.type, 'amount': this.util.ana.rawToAna(a.amount).toString(10),
